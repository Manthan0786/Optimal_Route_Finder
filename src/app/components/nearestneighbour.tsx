...
async function nearestNeighbor(coordinates: Coordinates[]): Promise<Coordinates[]> {
    ...
    while (!visited.every(value => value === true)) {
        ...
        for (let i = 0; i < coordinates.length; i++) {
            if (!visited[i]) {
                const distance = await calculateDistance(coordinates[currentCity], coordinates[i]);
